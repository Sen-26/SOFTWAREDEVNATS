...
class User(db.Model):
    ...
    avatar_path = db.Column(db.String(256), default='default.jpg')
    trash_collected = db.Column(db.Integer, default=0, nullable=False)
    coin = db.Column(db.Integer, default=0, nullable=False)
    unlocked_items = db.Column(MutableList.as_mutable(db.ARRAY(db.String)), default=lambda: ['map_1', 'banner_1'])
    equipped_items = db.Column(MutableList.as_mutable(db.ARRAY(db.String)), default=lambda: ['map_1', 'banner_1'])
